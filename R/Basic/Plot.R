...
createCumSumPnLPlot <- function(scores, labels, title, type, base_models) {
  ...
  plot <- ggplot(data, aes(x = dates, y = CumPnL)) + # x, y axis
    ...
    xlab("") + # format x-axis
    ylab("CumSum PnL") +
    scale_y_continuous(breaks = c(0, 500, 1000), limits = c(0, max(data$CumPnL * 1.1))) + # format y-axis
    theme_minimal() # theme
...
createSharpePPTPlot <- function(scores, labels, title, base_models, type, legendTitle) {
  ...
  bar1 <- ggplot(data = data, aes(
    x = Quantile, y = sharp, fill = method, width = 0.2,
    color = base_line
  )) +
    ...
    # x-axis
    ylab("Sharpe Ratio") +
    scale_y_continuous(limits = c(0, max(data$sharp + 0.5))) + # y-axis label
