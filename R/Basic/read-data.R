...
DatesToRemove <- function(Returns, PropStocksZero) {
  ...
  PropWithNoGrowth <- apply(Returns, 2, function(column) {
    mean(column == 0)
  })
...
StocksToRemove <- function(Returns, PropDaysZero, PropDaysUpperGrowth, UpperBoundGrowth) {
  ...
  PropWithNoGrowth <- apply(Returns, 1, function(row) {
    mean(row == 0)
  })
  ...
  PropHighGrowth <- apply(Returns, 1, function(row) {
    mean(row > UpperBoundGrowth)
  })
