...
Plot.Mean = ggplot(data, aes(x=alpha, y=Mean))+ geom_point(color = "#543005") +
  ylab("Mean PnL")  


Plot.Sd = ggplot(data, aes(x=alpha, y=sd))+ geom_point(color = "#543005") +
  ylab("Standard Deviation PnL")


Plot.Sharpe = ggplot(data, aes(x=alpha, y=Mean/sd * sqrt(252)))+ geom_point(color = "#543005") +
  ylab("Sharpe Ratio")
...
SharpePPT.OT.CT.OverAlpha = CreatePlot.SharpePPT(Scores=Scores[seq(1,15,by=2)],
                     Labels = Alpha.Candidates[seq(1,15,by=2)],
                     BaseModels = 1,
                     Type = "CT")


ggsave(filename = "SharpePPT.OT.CT.OverAlpha.png", 
       path = "C:\\Users\\Samuel Weinman\\OneDrive - Nexus365\\Documents\\MSc Statistical Science\\Dissertation\\Results\\Plots",
