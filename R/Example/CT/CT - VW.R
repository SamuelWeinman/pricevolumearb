...
Predictions.CT.VW.Divide <- list(
  Predictions.CT.VW.Divide[, 1:4433],
...
Scores.CT.VW.Divide <- performFullAnalysisFromList(Returns,
  Predictions.List = Predictions.CT.VW.Divide,
  Q = (1:4 / 4), r = 30
)

Scores.CT.VW.Divide.Combined <- append(list(Scores.CT), Scores.CT.VW.Divide)
names(Scores.CT.VW.Divide.Combined) <- c(
  "Not weighted", "x", "log(x)",
  "x^1/3", "sqrt(x)"
)


CumSum.CT.VW.Divide <- createCumSumPnLPlot(
  Scores = Scores.CT.VW.Divide.Combined,
  Labels = c(
...
SharpePPT.CT.VW.Divide <- createSharpePPTPlot(
  Scores = Scores.CT.VW.Divide.Combined,
  Labels = c(
    "Not Weighted (Baseline)",
    "V",
...
ggsave(
  filename = "CumSum.CT.VW.Divide.png",
...
ggsave(
  filename = "SharpePPT.CT.VW.Divide.png",
  path = "C:\\Users\\Samuel Weinman\\OneDrive - Nexus365\\Documents\\MSc Statistical Science\\Dissertation\\Results\\Plots",
...
Predictions.CT.VW.Multiply <- list(
  Predictions.CT.VW.Multiply[, 1:4433],
...
Scores.CT.VW.Multiply <- performFullAnalysisFromList(Returns,
  Predictions.List = Predictions.CT.VW.Multiply,
  Q = (1:4 / 4), r = 30
)
...
names(Scores.CT.VW.Multiply.Combined) <- c(
  "Not weighted", "x", "log(x)",
  "x^1/3", "sqrt(x)"
)
...
CumSum.CT.VW.Multiply <- createCumSumPnLPlot(
  Scores = Scores.CT.VW.Multiply.Combined,
  Labels = c(
    "Not Weighted (Baseline)",
    "V",
    "log(V)",
...
SharpePPT.CT.VW.Multiply <- createSharpePPTPlot(
  Scores = Scores.CT.VW.Multiply.Combined,
  Labels = c(
    "Not Weighted (Baseline)",
    "V",
...
ggsave(
  filename = "CumSum.CT.VW.Multiply.png",
  path = "C:\\Users\\Samuel Weinman\\OneDrive - Nexus365\\Documents\\MSc Statistical Science\\Dissertation\\Results\\Plots",
...
ggsave(
  filename = "SharpePPT.CT.VW.Multiply.png",
  path = "C:\\Users\\Samuel Weinman\\OneDrive - Nexus365\\Documents\\MSc Statistical Science\\Dissertation\\Results\\Plots",
